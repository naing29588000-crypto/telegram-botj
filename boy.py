...
FORWARD_CHANNEL = "https://Developer12920"
    ...
    if call.data.startswith("admin_"):
        ...
        if call.data == "admin_back":
            await bot.edit_message_text(
                ...
                message_id=call.message.message_id,
                text=f"🔐 **Admin Panel**\n\n"
                     f"⚡ Current Speed: `{MAX_CONCURRENT}` concurrent\n"
                     f"📦 Batch Size: `{BATCH_SIZE}`\n\n"
                     f"အောက်ပါ ခလုတ်များမှ သင်လိုချင်တာကို ရွေးချယ်ပါ။",
        ...
        if call.data == "admin_speed":
            await bot.edit_message_text(
                ...
                text=f"⚡ **Speed Setting**\n\n"
                     f"လက်ရှိ Speed: `{MAX_CONCURRENT}` concurrent\n"
                     f"Batch Size: `{BATCH_SIZE}`\n\n"
                     f"သင်ရွေးလိုသော Speed ကို နှိပ်ပါ -\n\n"
                     f"⚠️ **သတိပြုရန်:** Speed မြင့်လွန်းရင် Server Crash ဖြစ်နိုင်ပါတယ်။",
        ...
        if call.data.startswith("admin_speed_"):
            try:
                ...
                await bot.answer_callback_query(call.id, "❌ Speed Value မမှန်ပါ။")
                return

            MAX_CONCURRENT = new_speed
            db_set_setting("max_concurrent", new_speed)

            # Semaphore ကို reset လုပ်ပါ
            _voucher_sem = None

            await bot.edit_message_text(
                chat_id=chat_id,
                message_id=call.message.message_id,
                text=f"✅ **Speed Changed!**\n\n"
                     f"⚡ New Speed: `{MAX_CONCURRENT}` concurrent\n"
                     f"📦 Batch Size: `{BATCH_SIZE}`\n\n"
                     f"⚠️ **မှတ်ချက်:** လက်ရှိ Scan လုပ်နေတဲ့ Task တွေမှာ ဒီ Speed အသစ် မသက်ရောက်ပါ။\n"
                     f"Scan အသစ်စတဲ့အခါမှသာ အသစ်အလုပ်လုပ်ပါမယ်။",
                reply_markup=get_admin_back_keyboard(),
                parse_mode="Markdown"
            )
            await bot.answer_callback_query(call.id, f"⚡ Speed: {new_speed}")
            await forward_to_channel(f"⚡ Speed Changed to {new_speed} by Admin")
            return
...
{'' if auth_system.is_authenticated(user_id) else 'ကျေးဇူးပြု၍ /auth 1 thura ဖြင့် စတင်ပါ။'}"""

    await bot.reply_to(message, status_text, parse_mode="Markdown")

# ==================== FILE EXPORT ON STOP (FIXED) ====================
    ...
    if chat_id in success_texts and success_texts[chat_id]:
        try:
            ...
            try:
                ...
                print(f"Admin File Send Error: {e}")

            # အားလုံးပြီးမှ ဖိုင်ဖျက်
...
async def run_bruteforce(mode, chat_id, session_url, scan_id, message=None, progress_msg=None, start_digit=None):
    ...
    # Read current speed settings at scan start
    ...
    checked = 0
    retries = 0  # FIX: retries variable defined
    last_key_check = time.monotonic()
    scan_start = time.monotonic()

    # Create new semaphore with current speed for this scan
    ...
    try:
        while True:
            ...
            if total is not None:
                ...
                text = format_progress(checked, None, speed, found, retries)
